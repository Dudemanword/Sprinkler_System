...
function shaunscript(){
	console.log('Welcome to shaunscript');
}

function readPin(pin, callback){
	terminal_output('cat /sys/class/gpio/gpio'+pin_lookup[pin]+'/value', function(err, stdout, stderr){
			callback(stdout);
	});
}
...
function writePin(pin, value){
	console.log(value)
	terminal_output('echo out > /sys/class/gpio/gpio'+pin_lookup[pin]+'/direction', function(err,stdout,stderr){
	if(err) throw err;
		terminal_output('echo ' + value + ' > /sys/class/gpio/gpio'+pin_lookup[pin]+'/value', function(err,stdout,stderr){
			console.log('wrote to pin');
		});
	});
}

shaunscript.digitalWrite = function(pin, value){
	...
	file.exists('/sys/class/gpio/gpio' + pin_lookup[pin]+'/', function(exists){
		if(!exists){
			terminal_output('echo ' + pin_lookup[pin] + '> /sys/class/gpio/export', function(error, stdout, stderr){
				if (error) throw error;
				writePin(pin, value);
				});
			}
		else writePin(pin, value)
	});
}

shaunscript.digitalRead = function(pin, callback){
	file.exists('/sys/class/gpio/gpio' + pin_lookup[pin]+'/', function(exists){
		if(!exists){
			terminal_output('echo ' + pin_lookup[pin] + '> /sys/class/gpio/export', function(error, stdout, stderr){
				if (error) throw error;
