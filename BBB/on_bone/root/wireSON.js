...
wireless_scan.prototype.parseData = function(data, callback){
	...
	var bssid = 
/[0-9a-zA-z]{1,2}:[0-9a-zA-z]{1,2}:[0-9a-zA-z]{1,2}:[0-9a-zA-z]{1,2}:[0-9a-zA-z]{1,2}:[0-9a-zA-z]{1,2}/g
...
wireless_scan.prototype.generateJSON = function(SSID, security, sigLevelInt, frequencyInt, macAddress, callback){
	var jsonarr = [];
	for (stuff in SSID){
		jsonarr.push({SSID: SSID[stuff], Security_type: security[stuff], Sig_Strength: sigLevelInt[stuff], 
Freuency: frequencyInt[stuff], Mac_address: macAddress[stuff]});
		}
...
wireless_scan.prototype.scan = function(callback){
	...
	thisref.getData(function(data){
		thisref.parseData(data, function(SSID, security, sigLevelInt, frequencyInt, macAddress){
			thisref.generateJSON(SSID, security, sigLevelInt, frequencyInt, macAddress, 
function(isDone){
				callback(isDone);	
			});
		});
